...
async function updateAllStockDisplay() {
    ...
    if (currentStock.seed_stock && Array.isArray(currentStock.seed_stock)) {
        description += `**🌱 البذور:**\n`;
        currentStock.seed_stock.forEach(seed => {
            description += `• ${seed.display_name}: ${seed.quantity}\n`;
        });
        description += '\n';
    }
    ...
    if (currentStock.gear_stock && Array.isArray(currentStock.gear_stock)) {
        description += `**🛠️ الأدوات:**\n`;
        currentStock.gear_stock.forEach(gear => {
...
async function updateStockDisplay() {
    ...
    // إرسال رسالة جديدة في كل مرة
    try {
        await stockChannel.send({ embeds: [stockEmbed] });
    } catch (error) {
        console.error('[❌] خطأ في إرسال رسالة المخزون:', error);
    }
}
